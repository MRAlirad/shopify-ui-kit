...
const DeleteModal = ({ title, onClose, onDelete, isDeleting = false }: DeleteModalProps) => {
	...
	return createPortal(
		<div
			className="fixed inset-0 flex items-end md:items-center justify-center bg-black/60 z-[1500] backdrop-blur-sm"
			onClick={(e) => {
				if (e.target === e.currentTarget) onClose();
			}}
		>
			<div className="w-full max-w-md bg-white max-h-full p-4 rounded-lg overflow-hidden shadow animate-fadeInUp">
